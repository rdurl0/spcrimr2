...
Before installation, you will need these two packages:

```{r}
#install.packages("tidyverse")
library(tidyverse) # The Hadley Wickham's famous package for data science in R

# install.packages("devtools")
#devtools::install_github("abjur/abjMaps")
library(abjMaps)   # A package from Julio Trecenti wich helps a lot with cities names and mapping in Sao Paulo state
```

Finally, you can install the `spcrimr` from GitHub with: 
...
```{r}
seade_pop_variables_names %>% kableExtra::kable()
```
