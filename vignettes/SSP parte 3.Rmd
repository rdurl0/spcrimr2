...
ssp_ocorrencias_crimes <- map(dir(my_directory_path)[14:26], ~ paste0(my_directory_path, "/", .x)) %>%
...
```{r message=FALSE, warning=TRUE}
# loop
ssp_produtividade_policial  <- map(dir(my_directory_path)[1:13], ~ paste0(my_directory_path, "/", .x)) %>%
  map(~read_rds(.x)) %>% 
  map(~arrange(.x, Natureza, municipio)) %>% 
  map(~select(.x, municipio, ano, crime = Natureza, total = Total)) %>%
  map(~mutate(.x, #nm_municipio = nm_municipio,
