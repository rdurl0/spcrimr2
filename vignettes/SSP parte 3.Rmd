...
ocorrencias_crimes <- map(dir(my_directory_path)[14:26], ~ paste0(my_directory_path, "/", .x)) %>%
  ...
  modify_at(.x = ., .at = c(seq(8)), .f = ~select(.x, -lesao_corporal_seguida_de_morte)) %>%
  #???modify_at(.at = c(9:13), .f = ~mutate(.x, lesao_corporal_seguida_de_morte = c(rep(0, 645)))) %>%
...
```{r}
ocorrencias_crimes %>% str(max.level = 1)
```
...
```{r message=FALSE, warning=TRUE}
ocorrencias_crimes <- reduce(ocorrencias_crimes, ~bind_rows(.x, .y))
...
- O objeto `produtividade_policial` é o que vai armazenar as tabelas de ocorrencias de crime em um só `data.frame`.

```{r message=FALSE, warning=TRUE}
# loop
produtividade_policial  <- map(dir(my_directory_path)[1:13], ~ paste0(my_directory_path, "/", .x)) %>%
  map(~read_rds(.x)) %>% 
  map(~arrange(.x, Natureza, municipio)) %>% 
  map(~select(.x, municipio, ano, crime = Natureza, total = Total)) %>%
  map(~mutate(.x, #nm_municipio = nm_municipio,
...
```{r}
produtividade_policial %>% str(max.level = 1)
```

A função abaixo junta todos os anos em uma só tabela, o número de observações aumenta para $64\times13=8.385$:

```{r}
produtividade_policial <- reduce(produtividade_policial, ~bind_rows(.x, .y))

produtividade_policial
```
...
```{r}
painel <- inner_join(produtividade_policial,
                     ocorrencias_crimes,
...
```{r echo=FALSE}
variaveis <- c(
  colnames(produtividade_policial)[4:length(produtividade_policial)],
  colnames(ocorrencias_crimes)[4:length(ocorrencias_crimes)]
)
...
tibble(variaveis_tidy = variaveis,
       variaveis_raw  = titulos) %>% kableExtra::kable()
...
ssp <- function(x) {
  read_rds("./data/dados_ssp") %>%
  select(municipio, ano, x) %>%
  mutate(ano=as.factor(ano),
         municipio=as.factor(municipio))
}

ssp("homicidio_doloso")
```
...
```{r fig.align='center', fig.height=65, fig.width=12}
graficos <- map2(variaveis,
                 titulos, 
                 ~{ggplot(painel %>% filter(nm_municip == "SAO PAULO")) + 
                    geom_line(aes_string(x=c(2002:2014),
                                        y=paste0(.x))) +
                    theme_minimal() +
                    ggtitle(.y)
             })
